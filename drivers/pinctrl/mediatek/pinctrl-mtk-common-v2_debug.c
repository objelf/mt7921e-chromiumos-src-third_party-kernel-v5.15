	...
	struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf+len, bufLen-len,
		"PIN: (MODE)(DIR)(DOUT)(DIN)(DRIVE)(SMT)(IES)(PULL_EN)(PULL_SEL)[(R1 R0)/(RSEL)]\n");

	if (dbg_start >= chip->ngpio) {
		len += snprintf(buf+len, bufLen-len,
	...
	for (i = dbg_start; i < chip->ngpio; i++) {
		/* Do not print virtual PIN's information */
		if (hw->soc->pins[i].funcs[0].name == NULL)
			break;
		if (len > (bufLen - 96)) {
...
{
	int i, gpio, val, pullup = 0, pullen = 0;
	int vals[11];
	char attrs[11];
	struct mtk_pinctrl *hw = dev_get_drvdata(dev);
	const struct mtk_pin_desc *desc;
	...
	if (!strncmp(buf, "mode", 4)
		...
		if (val > 3) {
			pr_notice("invalid r1r0 parameter\n");
			goto out;
		}
		desc = (const struct mtk_pin_desc *)&hw->soc->pins[gpio];
		hw->soc->bias_get_combo(hw, desc, &pullup, &pullen);
		pullen = MTK_PUPD_SET_R1R0_00 + val;
		hw->soc->bias_set_combo(hw, desc, pullup, pullen);
		...
		if (val > 7) {
			pr_notice("invalid rsel parameter\n");
			goto out;
		}
		desc = (const struct mtk_pin_desc *)&hw->soc->pins[gpio];
		hw->soc->bias_get_combo(hw, desc, &pullup, &pullen);
		pullen = MTK_PULL_SET_RSEL_000 + val;
		hw->soc->bias_set_combo(hw, desc, pullup, pullen);
	} else if ((!strncmp(buf, "start", 5))
		&& (sscanf(buf+5, "%d", &val) == 1)) {
		dbg_start = val;
	} else if (!strncmp(buf, "set", 3)) {
		val = sscanf(buf+3, "%d %c%c%c%c%c%c%c%c%c%c %c", &gpio,
		...
		if (pullen < MTK_PUPD_SET_R1R0_00) {
			...
			pullen = MTK_PUPD_SET_R1R0_00 + vals[10];
			hw->soc->bias_set_combo(hw, desc, !!vals[9],
				pullen);
		} else {
			/* Case for: RSEL PIN */
			...
			pullen = MTK_PULL_SET_RSEL_000 + vals[10];
			hw->soc->bias_set_combo(hw, desc, !!vals[9],
				pullen);
		}
	} else {
		pr_notice("invalid input parameter\n");
		goto out;
	}
...
static DEVICE_ATTR(mt_gpio, 0664, mt_gpio_show, mt_gpio_store);
