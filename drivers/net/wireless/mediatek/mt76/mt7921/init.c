...
static int mt7921_check_offload_capability(struct mt7921_dev *dev)
{
	...
	dev_err(dev->mt76.dev, "*[[[[year = %d , mon = %d, day = %d, hour = %d, min = %d, sec = %d]]]\n",
		year, mon, day, hour, min, sec);

	/* Old firmware cannot support remained on channel and chanctx*/
	fw_can_roc =  mktime64(year, mon, day, hour, min, sec) >=
		      mktime64(2022, 6, 8, 21, 8, 39);
...
mt7921_init_wiphy(struct ieee80211_hw *hw)
{
	...
	hw->queues = 4;
	hw->max_rx_aggregation_subframes = 0x100;
	hw->max_tx_aggregation_subframes = 0x100;
